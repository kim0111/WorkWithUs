...
async def get_review(reviewer_id: int, project_id: int, review_type: str,
                     reviewee_id: int | None = None) -> Review | None:
    filters = {"reviewer_id": reviewer_id, "project_id": project_id, "review_type": review_type}
    if reviewee_id is not None:
        filters["reviewee_id"] = reviewee_id
    return await Review.filter(**filters).first()
